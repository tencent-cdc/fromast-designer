...
const attrTypes = [
  ...
  { value: VALUE_TYPES.ENUM, text: '枚举值' },
...
export function RichPropEditor(props) {
  ...
  return (
    <>
      ...
      {data.type === VALUE_TYPES.EXP || data.type === VALUE_TYPES.FN ? <Textarea value={data.value} onChange={(e) => onChange(produce(data, data => { data.value = e.target.value }))} disabled={data.disabled} /> : null}
    </>
  )
}
